...
#define HW_REGS (sizeof (map_hw_reg_to_dwarf_reg) / sizeof (int))
...
static int map_dwarf_reg_to_hw_reg [DWARF_REGS];
...
typedef struct {
	Loc locations [HW_REGS];
	guint8 reg_saved [HW_REGS];
				   ...
				   guint8 **out_cfa)
{
	...
	for (hwreg = 0; hwreg < HW_REGS; ++hwreg) {
