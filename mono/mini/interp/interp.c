...
get_virtual_method (MonoDomain *domain, RuntimeMethod *runtime_method, MonoObject *obj)
{
	...
	if (m->is_inflated && mono_method_get_context (m)->method_inst) {
		...
		if (virtual_method->klass->is_inflated) {
		...
		} else if (virtual_method->klass->generic_container) {
...
#endif
	// g_print ("stack size: %d\n", ((guint32) frame) - ((guint32) &child_frame));
...
#define LDC(n) do { sp->data.i = (n); ++ip; ++sp; } while (0)
		...
		MINT_IN_CASE(MINT_JIT_CALL) {
			...
			MonoFtnDesc ftndesc;
			MonoType *type;
			guint8 *res_buf = malloc (sizeof (gpointer) );
...
ves_exec (MonoDomain *domain, MonoAssembly *assembly, int argc, char *argv[])
{
	...
	rval = mono_runtime_run_main (method, argc, argv, NULL);
	return rval;
}
